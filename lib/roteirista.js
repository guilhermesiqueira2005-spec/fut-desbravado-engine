...
function reacaoAoVivo(match, minuto, social) {
...
function analisePosJogo(match, social) {
  const lesao = lesaoDe(match.mandante) || lesaoDe(match.visitante);
  const d = duelo(match);
  const cenas = [
    cena(
...
function montar(match, formato, titulo, cenas, hashtags, extra = {}) {
  ...
  return {
    ...
    hashtags,
    drive: { pasta: drivePath(match), arquivoBase: `${formato.toLowerCase()}_jogo${match.id}` },
