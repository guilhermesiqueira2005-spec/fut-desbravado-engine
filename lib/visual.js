...
function placarOverlay({ mandante, visitante, placar = null, minuto = "", status = "" }) {
  // sem placar real confirmado, mostra "VS" — nunca um 0 x 0 inventado
  const tem = placar !== null && placar !== undefined && placar !== "";
  const [g1, g2] = tem ? String(placar).split(/x|×/).map((s) => s.trim()) : ["", ""];
  const centro = tem ? `${esc(g1)} – ${esc(g2)}` : "VS";
  ...
  <text x="450" y="86" text-anchor="middle" font-family="Segoe UI,Arial" font-size="46" font-weight="900" fill="#ffffff">${centro}</text>
