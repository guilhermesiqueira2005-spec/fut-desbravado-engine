...
async function despacharParaMake(resultado) {
  ...
  if (!url) return { despachado: false, motivo: "MAKE_WEBHOOK_URL não configurada" };
  const acoes = resultado.acoes.filter((a) => MAKE_FORMATS.includes(a.tipo));
  if (!acoes.length) return { despachado: false, motivo: "sem ações renderizáveis neste tick" };
  const res = await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ ...resultado, acoes }),
  });
  return { despachado: res.ok, status: res.status, eventos: acoes.length };
}
